(function (global, factory) {
	...
	var sha1$1=function(string){return sha1(string).toString()};function setLocalStorage(key,value,expired){return value||0===value?key||0===key?(window.localStorage.setItem(sha1$1(key),encBase64.stringify(_JSON$stringify(value))),expired&&_Number$isInteger(expired)&&window.localStorage.setItem(sha1$1("".concat(key,"__expires__")),encBase64.stringify(_Date$now()+1e3*expired)),value):"":""}function getLocalStorage(key){var time=window.localStorage.getItem(sha1$1("".concat(key,"__expires__")));if(!time)return "";var expired=encBase64.parse(time),now=_Date$now();if(expired&&now>=_parseInt(expired,10))return removeLocalStorage(sha1$1(key)),"";var data=window.localStorage.getItem(sha1$1(key));if(!data)return "";var value=encBase64.parse(data);return value?JSON.parse(value):null}function removeLocalStorage(key){return key||0===key?void(window.localStorage.removeItem(sha1$1(key)),window.localStorage.removeItem(sha1$1("".concat(key,"__expires__")))):""}
...
//# sourceMappingURL=ciel-localstorage.js.map
