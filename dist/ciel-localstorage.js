(function (global, factory) {
	...
	var sha1$1=function(string){return sha1(string).toString()};function setLocalStorage(key,value,expired){return value||0===value?(window.localStorage.setItem(sha1$1(key),encBase64.stringify(_JSON$stringify(value))),expired&&_Number$isInteger(expired)&&window.localStorage.setItem(sha1$1("".concat(key,"__expires__")),encBase64.stringify(_Date$now()+1e3*expired)),value):""}function getLocalStorage(key){var expired=encBase64.parse(window.localStorage.getItem(sha1$1("".concat(key,"__expires__")))),now=_Date$now();if(expired&&now>=_parseInt(expired,10))return removeLocalStorage(sha1$1(key)),"";var value=encBase64.parse(window.localStorage.getItem(sha1$1(key)));return value?JSON.parse(value):null}function removeLocalStorage(key){window.localStorage.removeItem(sha1$1(key)),window.localStorage.removeItem(sha1$1("".concat(key,"__expires__")));}
